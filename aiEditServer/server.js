...
class BusinessServer {
    ...
    constructor(options = {}) {
        // 设置环境变量
        process.env.IMAGEAI_API_KEY = 'sk-4606777c987';
    ...
    init() {
        ...
        this._cleanupService = new CleanupService({
            ...
            uploadSubDir: this.options.uploadSubDir,
            resultsSubDir: this.options.resultsSubDir,
    ...
    _setupMiddlewares() {
        ...
        this._ownershipMiddleware = new OwnershipMiddleware({ 
            baseDir: this.options.baseDir,
            uploadSubDir: this.options.uploadSubDir 
        });
        ...
        files.forEach(fileData => {
            this._processFile(fileData, effectiveUserId, (result) => {
                ...
                if (result.success && result.savedAs && this._isImageFile(result.savedAs)) {
                    const userUploadDir = this._fileService.getUserUploadDir(effectiveUserId);
...
_processSingleImage(externalUrl, filename, modelConfig, filePath) {
    ...
    this._imageAI.process([externalUrl], modelConfig, (error, results) => {
        ...
        if (results[externalUrl]) {
            this._imageAI.results[filename] = {
                ...
                model: modelConfig.model, // 保存使用的模型
                prompt: modelConfig.prompt, // 保存使用的提示词
                parameters: modelConfig.parameters // 保存使用的参数
            };
    ...
    _processFile(fileData, userId, callback) {
        ...
        const userUploadDir = this._fileService.getUserUploadDir(userId);
        ...
        file.mkdir(userUploadDir, (mkdirErr) => {
            ...
            file.write(filePath, contentBuffer, (err) => {
                ...
                if (!err) {
                    result.externalUrl = `${this.options.externalBaseUrl}/${userId}/uploads/${safeFilename}`;
                }
            ...
            file.ls(userUploadDir, (err, files) => {
                ...
                const fileList = files
                    ...
                    .map(item => ({
                        ...
                        externalUrl: `${this.options.externalBaseUrl}/${userId}/uploads/${item.name}`
                    }));
     ...
     */
    _cleanupExpiredResults(userId = 'anonymous') {
        try {
            // 使用清理服务清理过期结果
            this._cleanupService.performAllCleanup(userId, this._imageAI.results, (err, results) => {
                if (err) {
     ...
     */
    _performAllCleanup() {
        // 对每个用户执行清理，这里可以遍历所有用户目录
        // 但为了简单起见，只清理 anonymous 用户
        this._cleanupService.performAllCleanup('anonymous', this._imageAI.results, (err, results) => {
            if (err) {
    ...
    _ensureUploadDir() {
        // 只创建基础目录，用户目录按需创建
        file.mkdir(this.options.baseDir, (err) => {
            if (err && err.code !== 'EEXIST') {
                console.error(`创建基础目录失败: ${err.message}`);
                return;
            }
            console.log(`确保基础目录存在: ${this.options.baseDir}`);
        });
    }
    ...
    start(callback) {
        ...
        console.log(`文件存储基础目录: ${this.options.baseDir}`);
        console.log(`目录结构: 用户目录/${this.options.uploadSubDir} 和 用户目录/${this.options.resultsSubDir}`);
