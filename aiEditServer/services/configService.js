...
class ConfigService {
    ...
    constructor(defaultConfig = {}) {
        this.config = {
            ...
            // 文件存储配置 - 新的目录结构：用户目录/uploads
            baseDir: './360house-master',  // 基础目录
            uploadSubDir: 'uploads',  // 上传文件子目录
            resultsSubDir: 'results',  // 处理结果子目录
    ...
    getUserDir(userId) {
        ...
        return path.join(this.config.baseDir, userId || 'anonymous');
    }
    ...
    getUserUploadDir(userId) {
        const path = require('path');
        return path.join(this.getUserDir(userId), this.config.uploadSubDir);
    }
    ...
    getUserResultsDir(userId) {
        const path = require('path');
        return path.join(this.getUserDir(userId), this.config.resultsSubDir);
    }
}
