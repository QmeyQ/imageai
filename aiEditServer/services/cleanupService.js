...
class CleanupService {
     ...
     * @param {string} options.uploadSubDir - 上传文件子目录
     * @param {string} options.resultsSubDir - 处理结果子目录
     ...
     * @param {Function} callback - 回调函数 (error, cleanedCount)
     */
    cleanupUnprocessedImages(userId, imageAIResults, callback) {
        const now = new Date();
        const unprocessedExpiryTime = this.options.unprocessedImageExpiryHours * 60 * 60 * 1000;
        
        const userUploadDir = path.join(this.options.baseDir, userId || 'anonymous', this.options.uploadSubDir);
        const baseDir = path.resolve(this.options.baseDir);
        const resolvedDir = path.resolve(userUploadDir);
        ...
        file.ls(userUploadDir, (err, files) => {
            if (err) {
                ...
                console.error(`读取用户 ${userId} 的上传目录失败:`, err);
            ...
            files.forEach(fileInfo => {
                ...
                if (fileInfo.type === 'file') {
                    ...
                    // 检查文件是否在结果中存在
                    if (!imageAIResults[filePath] && !imageAIResults[filename]) {
                        // 文件不在结果中，可能是未处理的图片
                            ...
                            if (fileAge > unprocessedExpiryTime) {
                                file.del(filePath, (delErr) => {
                                    ...
                                    if (delErr) {
                                        ...
                                        cleanedCount++;
                                        console.log(`已删除用户 ${userId} 的未处理图片: ${filePath}`);
                                    }
     ...
     */
    performAllCleanup(userId, imageAIResults, callback) {
        let totalCleaned = 0;
        let completedTasks = 0;
        const results = {
            expiredResults: 0,
            unprocessedImages: 0,
            expiredResultFiles: 0
        };
        ...
        // 清理未处理图片
        this.cleanupUnprocessedImages(userId, imageAIResults, (err, count) => {
            if (err) {
        ...
        // 清理过期结果文件
        this.cleanupExpiredResultFiles(userId, (err, count) => {
            if (err) {
                console.error('清理过期结果文件时出错:', err);
            } else {
                results.expiredResultFiles = count;
                totalCleaned += count;
            }
     ...
     * @param {Function} callback - 回调函数 (error, cleanedCount)
     */
    cleanupExpiredResultFiles(userId, callback) {
        const now = new Date();
        const expiryTime = this.options.resultExpiryDays * 24 * 60 * 60 * 1000;
        
        // 构建用户特定的结果目录
        const userResultsDir = path.join(this.options.baseDir, userId || 'anonymous', this.options.resultsSubDir);
        
        file.ls(userResultsDir, (err, files) => {
            if (err) {
                // 如果目录不存在，不报错
                if (err.code === 'ENOENT') {
                    return callback(null, 0);
                }
                console.error(`读取用户 ${userId} 的结果目录失败:`, err);
                return callback(err, 0);
            }
            ...
            files.forEach(fileInfo => {
                if (fileInfo.type === 'file') {
                    const filePath = path.join(userResultsDir, fileInfo.name);
                    const fileMtime = fileInfo.mtime;
                    ...
                    if (fileMtime) {
                        ...
                        if (fileAge > expiryTime) {
                            file.del(filePath, (delErr) => {
                                processedCount++;
                                if (delErr) {
                                    console.error(`删除过期结果文件失败 ${filePath}:`, delErr);
                                } else {
                                    cleanedCount++;
                                    console.log(`已删除用户 ${userId} 的过期结果文件: ${filePath}`);
                                }
