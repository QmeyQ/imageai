...
class FileService {
     ...
     */
    isSafePath(filePath, userId = null) {
        // 检查路径是否在基础目录中
        const normalizedBaseDir = path.resolve(this.options.baseDir);
     ...
     */
    getUserDir(userId) {
        return path.join(this.options.baseDir, userId || 'anonymous');
    }
    ...
    getUserUploadDir(userId) {
        return path.join(this.getUserDir(userId), this.options.uploadSubDir);
    }

    /**
     * 获取用户结果目录
     * @param {string} userId - 用户ID
     * @returns {string} 结果目录路径
     */
    getUserResultsDir(userId) {
        return path.join(this.getUserDir(userId), this.options.resultsSubDir);
    }
