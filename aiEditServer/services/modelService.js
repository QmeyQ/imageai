...
class ModelService {
    ...
    _loadModels() {
        try {
            ...
            // 提取元数据（如果有）
            if (config._meta) {
                this.meta = config._meta;
                delete config._meta; // 从模型列表中移除元数据
            } else {
                this.meta = { version: '1.0.0', lastUpdated: new Date().toISOString().split('T')[0] };
            }
     ...
     */
    getConfigVersion() {
        // 优先使用models.json中定义的版本
        if (this.meta && this.meta.version) {
