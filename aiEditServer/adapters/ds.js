...
class DashScopeAdapter {
    ...
    invoke(model, op, input, cfg, callback) {
        ...
        // 万象2.5模型使用不同的输入格式
        let payload;
        if (modelStr === 'wan2.5-i2i-preview') {
            // 万象2.5使用images数组和prompt直接在input中
            payload = {
                model: modelStr,
                input: {
                    prompt: input.prompt,
                    images: input.images || [input.base_image_url],
    ...
    getOperations(model, callback) {
        ...
        const operations = {
            ...
            'wan2.5-i2i-preview': ['image_edit', 'multi_reference_generation']
        };
